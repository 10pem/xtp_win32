...
def preprocess(line):
    ...
    # 去除行尾注释
    if cppline[:2] != '//' and '//' in cppline:
        cppline = cppline.split('///')[0].strip(' ')
    return cppline
...
def handle_end(cppline, structname):
    pyline = 'structDict[\'{}\'] = {}\n\n\n\n'.format(structname, structname)
    if len(cppline)>1:
        abbreviation = cppline.split(' ')[1]
        if abbreviation != structname:
            pyline += '\nstructDict[\'{}\'] = {}\n\n\n'.format(abbreviation, structname)
    return pyline
...
def main(f1, f2):
    ...
    for linenum, line in enumerate(lines):
        ...
        if cppline.startswith('//'):
            ...
            pyline = handle_end(cppline, structname)
        # 为xquote_api_struct.h做的处理
        elif cppline.startswith('union'):
            pyline = ''
...
if __name__ == '__main__':
    f1 = 'xtp\\xquote_api_struct.h'
    f2 = 'ojbk\\xquote_api_struct.py'
    main(f1, f2)
